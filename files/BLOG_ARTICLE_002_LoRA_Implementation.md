...
category: "機械学習"
tags: ["LoRA", "Fine-tuning", "VLM", "LLaVA", "HuggingFace", "PEFT", "Visual RAG"]
date: "2026-03-21"
...
この LoRA アダプタは、**Visual RAG (CLIP) + Agentic RAG** パイプラインの中核として、画像理解とマルチモーダル検索を支えています。Cloud Run 上の本番環境では CPU モードでモックフォールバックが動作し、GPU 環境では完全な推論が可能です。

```
【このステップの価値】
✅ Google Colab の無料 GPU で実行可能
✅ 3,000 サンプルで効果的に学習（Loss=0.969）
✅ HuggingFace で公開済み（Shion1124/vlm-lora-agentic-rag）
✅ 本番環境（Cloud Run）での推論に対応
✅ Visual RAG + Agentic RAG パイプラインの中核として稼働
```

次の記事では、「**Agentic RAG とは何か｜複数戦略による自律検索**」で、このモデルを使用した高度な検索パイプライン（Visual RAG 統合を含む）の構築方法を解説します。
...
- 2026-03-23：Visual RAG パイプラインとの統合情報を追記、Cloud Run 本番対応を反映
- 2026-03-21：初版公開
