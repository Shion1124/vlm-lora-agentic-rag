...
description: "Agentic RAG の仕組みを完全解説。キーワード検索、セマンティック検索、ハイブリッド検索の統合、Visual RAG (CLIP) とのマルチモーダル検索、信頼度ベースの自動選択メカニズムを実装例とともに紹介。"
category: "機械学習"
tags: ["RAG", "Agentic RAG", "Visual RAG", "CLIP", "FAISS", "BM25", "検索", "情報取得", "マルチモーダル"]
date: "2026-03-21"
...
さらに本プロジェクトでは、**Visual RAG (CLIP)** を統合し、テキストだけでなく画像も検索対象とする **マルチモーダル検索** を実現しています。

> **このアプローチの利点**  
> ✅ クエリの複雑性に自動対応  
> ✅ 検索精度 87% に向上（従来 72%）  
> ✅ 幻覚（Hallucination）65% 削減  
> ✅ Visual RAG (CLIP) で画像検索も対応 🆕  
> ✅ 推論時間は変わらず高速
...
- [実装コード全体](#実装コード全体)
- [Visual RAG との統合](#Visual-RAG-との統合)
- [パフォーマンス評価](#パフォーマンス評価)
...
本プロジェクトでは、テキストベースの Agentic RAG に加え、**Visual RAG (CLIP)** を統合することで、テキストと画像の両方を横断するマルチモーダル検索を実現しています。

```python
# マルチモーダル検索の実装例
class DocumentStructuringPipeline:
    def __init__(self):
        self.visual_rag = VisualRAGEngine()   # CLIP + FAISS
        self.rag = AgenticRAGEngine()          # Sentence-T + BM25 + FAISS
    
    def multimodal_search(self, query, top_k=5):
        """テキスト検索 + 画像検索を統合"""
        # テキスト検索（Agentic RAG）
        text_results = self.rag.search(query, top_k=top_k)
        
        # 画像検索（Visual RAG）
        visual_results = self.visual_rag.search_by_text(query, top_k=top_k)
...
```
【Visual RAG (CLIP) の仕組み】

1. ドキュメント登録時:
   画像ファイル → CLIP Image Encoder → 512次元ベクトル → FAISS Index

2. 検索時:
   テキストクエリ → CLIP Text Encoder → 512次元ベクトル
   → FAISS で最近傍探索 → 類似画像を取得
...
```bash
# テキスト検索のみ（Agentic RAG）
curl -X POST /search \
  -H "X-API-Key: your-key" \
  -d '{"query": "売上150億のリスク"}'

# マルチモーダル検索（Visual RAG + Agentic RAG）
curl -X POST /multimodal-search \
  -H "X-API-Key: your-key" \
  -d '{"query": "売上トレンドのグラフ", "top_k": 5}'
```
...
- 2026-03-21：初版公開
- 2026-03-23：Visual RAG (CLIP) との統合セクション追加、マルチモーダル検索記述追加
